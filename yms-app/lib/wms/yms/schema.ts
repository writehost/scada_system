...
export async function ensureYmsSchema(client: PoolClient): Promise<void> {
  if (!schemaReady) {
...
/** Стартовый план, только если у площадки ещё нет объектов. Не трогает остатки WMS. */
export async function ensureYardSeed(client: PoolClient, siteId: number): Promise<void> {
...
export function resetYmsSchemaCacheForTests(): void {
  schemaReady = false
}
