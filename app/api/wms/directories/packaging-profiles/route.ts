  ...
  try {
    ...
    const result = await client.query<{
      ...
      SELECT
        ...
        created_at::text,
        updated_at::text
      FROM wms_packaging_profile_defs
      WHERE site_id = $1
      ORDER BY sort_order, profile_code
      `,
      [siteId]
    );
    return NextResponse.json({
      profiles: result.rows.map((row) => rowToDto(row)),
    });
  } catch (e) {
...
export async function POST(req: Request) {
  ...
  if (!/^[a-z0-9_]{1,64}$/.test(code)) {
    return NextResponse.json(
      { error: "code: только латиница, цифры и _, до 64 символов" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const ins = await client.query<{
      ...
      `
      INSERT INTO wms_packaging_profile_defs (
        site_id, profile_code, name, description, sort_order, is_active, created_at, updated_at
      )
