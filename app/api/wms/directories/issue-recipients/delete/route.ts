...
import { deleteIssueRecipientDef } from "@/lib/wms/issue-recipient-directory";
...
/** POST вместо DELETE /[recipientCode] — в production Next.js dynamic app routes отдают 500. */
export async function POST(req: Request) {
  ...
  try {
    ...
    const deleted = await deleteIssueRecipientDef(client, siteId, recipientCode);
    if (!deleted) {
      return NextResponse.json({ error: "not found" }, { status: 404 });
    }
    return NextResponse.json({ ok: true, recipientCode });
  } catch (e) {
