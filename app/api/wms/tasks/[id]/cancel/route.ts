...
import { cancelTask } from "@/lib/wms/tasks";
...
export async function POST(
  ...
  let body: { requestId?: string; siteCode?: string; note?: string };
  try {
    body = (await req.json()) as { requestId?: string; siteCode?: string; note?: string };
  } catch {
  ...
  try {
    const result = await runIdempotentWrite(
      ...
      "task_cancel",
      { ...body, taskId },
      (client) => cancelTask(client, siteId, taskId, body.note)
    );
