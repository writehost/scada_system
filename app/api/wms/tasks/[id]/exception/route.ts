...
import { reportTaskException } from "@/lib/wms/tasks";
...
export async function POST(
  ...
  const exceptionCode =
    typeof body.exceptionCode === "string" ? body.exceptionCode.trim() : "";
  if (!requestId || !siteCode.trim() || !taskId.trim() || !exceptionCode) {
    return NextResponse.json(
      { error: "requestId, siteCode, task id and exceptionCode are required" },
      { status: 400 }
    );
  }
  ...
  try {
    const result = await runIdempotentWrite(
      ...
      { ...body, taskId },
      (client) =>
        reportTaskException(client, siteId, taskId, exceptionCode, body.exceptionNote)
    );
