...
import { claimTask } from "@/lib/wms/tasks";
...
export async function POST(
  ...
  let body: { requestId?: string; siteCode?: string; assignedUserId?: string };
  try {
    body = (await req.json()) as { requestId?: string; siteCode?: string; assignedUserId?: string };
  } catch {
  ...
  try {
    const result = await runIdempotentWrite(
      ...
      "task_claim",
      { ...body, taskId },
      (client) => claimTask(client, siteId, taskId, body.assignedUserId)
    );
