...
import { reportTaskExceptionByDevice } from "@/lib/wms/tasks";
...
export async function POST(
  ...
  const exceptionCode = typeof body.exceptionCode === "string" ? body.exceptionCode : "";
  if (!requestId || !siteCode.trim() || !taskId.trim() || !deviceUid.trim() || !exceptionCode.trim()) {
    return NextResponse.json(
      { error: "requestId, siteCode, taskId, deviceUid and exceptionCode are required" },
      { status: 400 }
    );
  }
  ...
  try {
    const result = await runIdempotentWrite(
      ...
      requestId,
      siteId,
      "task_exception_device",
      { ...body, taskId },
      (client) =>
        assertDeviceMutationAllowed(client, siteId, deviceUid, body.operatorUserId, taskId).then(() =>
          reportTaskExceptionByDevice(
            client,
            siteId,
            taskId,
            deviceUid,
            exceptionCode,
            typeof body.exceptionNote === "string" ? body.exceptionNote : undefined
          )
        )
    );
