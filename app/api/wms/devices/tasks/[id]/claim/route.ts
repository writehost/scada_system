...
import { claimTaskByDevice } from "@/lib/wms/tasks";
import { runIdempotentWrite } from "@/lib/wms/idempotency";
import { parseRequestId } from "@/lib/wms/uuid";
import { WmsHttpError } from "@/lib/wms/errors";
import { assertDeviceClaimAllowed } from "@/lib/wms/device-operator-auth";
...
export async function POST(
  ...
  try {
    const result = await runIdempotentWrite(
      ...
      { ...body, taskId },
      (client) =>
        assertDeviceClaimAllowed(client, siteId, deviceUid, body.operatorUserId, taskId).then(() =>
          claimTaskByDevice(client, siteId, taskId, deviceUid)
        )
    );
