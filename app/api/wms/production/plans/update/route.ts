...
import { updateProductionPlan } from "@/lib/wms/production-plan-directory";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const plan = await updateProductionPlan(client, siteId, planCode, {
      planDate: body.planDate,
      planDateTo: body.planDateTo,
      itemCode: body.itemCode,
      plannedQty: body.plannedQty != null ? Number(body.plannedQty) : undefined,
      workshopCode: body.workshopCode,
      lineCode: body.lineCode,
      materialWarehouseCode: body.materialWarehouseCode,
      note: body.note,
      status: body.status as Parameters<typeof updateProductionPlan>[3]["status"],
